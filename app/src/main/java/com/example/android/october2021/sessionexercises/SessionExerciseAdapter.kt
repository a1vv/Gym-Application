...
class SessionExerciseAdapter(val clickListener: SessionExerciseListener):
    ListAdapter<SessionExercise, SessionExerciseAdapter.SessionExerciseHolder>(
    SessionExerciseDiffCallback()
    ) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SessionExerciseHolder {
    ...
    class SessionExerciseHolder(private val binding: SessionExerciseItemBinding): RecyclerView.ViewHolder(binding.root) {

        fun bind(item: SessionExercise, clickListener: SessionExerciseListener){
...
// returns the id of the clicked SessionExercise
class SessionExerciseListener(val clickListener: (sessionExerciseId: Long) -> Unit){
    fun onClick(sessionExercise: SessionExercise) = clickListener(sessionExercise.sessionExerciseId)
}